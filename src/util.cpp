...
int nAnonymizeBackpocketAmount = 1000;
...
#else
    const char* pszModule = "Backpocket";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Backpocket
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Backpocket
    // Mac: ~/Library/Application Support/Backpocket
    // Unix: ~/.Backpocket
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Backpocket";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Backpocket.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "backpocketd.pid"));
