...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBackpocketAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBackpocketAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBackpocketAmount = strAnonymizeBackpocketAmount.remove(strAnonymizeBackpocketAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBackpocketAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBackpocketAmount*COIN) nMaxToAnonymize = nAnonymizeBackpocketAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBackpocketAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBackpocketAmount));
        strAnonymizeBackpocketAmount = strAnonymizeBackpocketAmount.remove(strAnonymizeBackpocketAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBackpocketAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBackpocketAmount == 0){
