...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Backpocket"))
        return false;
...
{
    // Convert Backpocket:// to Backpocket:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Backpocket://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Backpocket:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Backpocket.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Backpocket.lnk
...
{
    return GetAutostartDir() / "Backpocket.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Backpocket.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Backpocket\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Backpocket app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Backpocket app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Backpocket-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Backpocket-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Backpocket-Qt"));
