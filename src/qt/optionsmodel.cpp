...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBackpocketAmount"))
        settings.setValue("nAnonymizeBackpocketAmount", 1000);
    nAnonymizeBackpocketAmount = settings.value("nAnonymizeBackpocketAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBackpocketAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBackpocketAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBackpocketAmount:
            return QVariant(nAnonymizeBackpocketAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBackpocketAmount:
            nAnonymizeBackpocketAmount = value.toInt();
            settings.setValue("nAnonymizeBackpocketAmount", nAnonymizeBackpocketAmount);
            emit AnonymizeBackpocketAmountChanged(nAnonymizeBackpocketAmount);
            break;
        default:
            break;
        }
