...
void Shutdown()
{
    ...
    RenameThread("Backpocket-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Backpocket.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: backpocketd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebackpocketamount=<n> " + _("Keep N Backpocket anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Backpocket is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Backpocket is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Backpocket version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Backpocket server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Backpocket") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Backpocket to complete") << "\n";
    ...
    nAnonymizeBackpocketAmount = GetArg("-anonymizebackpocketamount", 0);
    if(nAnonymizeBackpocketAmount > 999999) nAnonymizeBackpocketAmount = 999999;
    if(nAnonymizeBackpocketAmount < 2) nAnonymizeBackpocketAmount = 2;
    ...
    LogPrintf("Anonymize Backpocket Amount %d\n", nAnonymizeBackpocketAmount);
